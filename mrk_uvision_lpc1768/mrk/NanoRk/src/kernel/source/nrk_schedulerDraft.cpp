...
void _nrk_scheduler() {
    ...
    // if current task is suspended remove from ready cue
    ...
    if (nrk_cur_task_TCB->cpu_reserve != 0 && nrk_cur_task_TCB->task_ID != NRK_IDLE_TASK_ID && 
			      nrk_cur_task_TCB->task_state != SUSPENDED && nrk_cur_task_TCB->task_state != FINISHED) {
					// _nrk_prev_timer_val is actually less than or equal to cpu_remaining by design
					// this is before update of cpu_remaining
        ...
        task_ID = nrk_cur_task_TCB->task_ID;
				...
				if (nrk_cur_task_TCB->cpu_remaining == 0) {
						nrk_cur_task_TCB->task_state = SUSPENDED;
						nrk_rem_from_readyQ(nrk_cur_task_TCB->task_ID);
					// add a line here to check set the suspend flag to 1 if task is suspended
    ...
    for (task_ID = 0; task_ID < NRK_MAX_TASKS; task_ID++) {
        ...
        if (nrk_task_TCB[task_ID].task_ID != NRK_IDLE_TASK_ID  && nrk_task_TCB[task_ID].task_state != FINISHED) {
            ...
            // Hint: next_wakeup is the number of ticks before the task will be woken up again
						nrk_task_TCB[task_ID].next_wakeup -= (uint16_t)_nrk_prev_timer_val;
            ...
            // Hint: next_period is the number of ticks until the next period for a task starts
						nrk_task_TCB[task_ID].next_period -= (uint16_t)_nrk_prev_timer_val;
        ...
        if (nrk_task_TCB[task_ID].task_state == SUSPENDED) {
            ...
            if (nrk_task_TCB[task_ID].next_wakeup == 0) {
                ...
                // For next_wakeup, think about the next time we'd want to add this task to the ready queue
								if ( !(nrk_task_TCB[task_ID].cpu_remaining == 0 && nrk_task_TCB[task_ID].next_period == 0)){
											printf("TE: %d", nrk_task_TCB[task_ID].task_ID);
									}
                nrk_task_TCB[task_ID].cpu_remaining = nrk_task_TCB[task_ID].cpu_reserve;
                nrk_task_TCB[task_ID].task_state    = READY;
                nrk_task_TCB[task_ID].next_wakeup   = nrk_task_TCB[task_ID].period;
		nrk_task_TCB[task_ID].next_period   = nrk_task_TCB[task_ID].next_period;
